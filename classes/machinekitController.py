...
class MachinekitController():
    ...
    def manual_control(self, axes, speed, increment):
        ...
        if not type(axes) == int:
            ...
            if not axes.isdecimal():
                raise ValueError({"message": "Axe must be an integer or convertable to an integer", "status": 400, "type": "ValueError"})
        if not type(speed) == float:
            if not speed.replace(".", "").isdecimal():
                raise ValueError({"message": "Speed must be a float or convertable to a float", "status": 400, "type": "ValueError"})
    ...
    def spindle(self, command):
        if type(command.values()[0]) is not unicode:
            raise Exception({"message": "Command must be a string", "status": 400, "type": "ValueError"})
